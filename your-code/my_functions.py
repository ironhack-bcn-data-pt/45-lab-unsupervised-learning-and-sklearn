...
def detect_outliers(df,column):
    ...
    print(f'original number of rows: {len(df)}')
    out_df = df
    out_df['zscore'] = zscore(out_df[column])
    df['is_outlier'] = df['zscore'].apply(lambda x: x <= -1.96 or x >= 1.96)
    out_rows = df[df['is_outlier'] == True].index
    out_df.drop('zscore',axis=1,inplace=True)
    out_df.drop('is_outlier',axis=1,inplace=True)
    print(f'processed number of rows: {len(out_rows)}')
    return out_rows